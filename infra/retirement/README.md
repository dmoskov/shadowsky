...
- Credential follow-up: the user confirmed the Anthropic credentials have no shared consumers and authorized deletion. The sandbox `ANTHROPIC_API_KEY` SSM parameter was deleted; `shadowsky/anthropic-api-key` was removed from Terraform and scheduled for deletion using its existing seven-day recovery window. The secret is unavailable to normal retrieval during that window. A subsequently authorized read-only authentication check found one distinct archived key shared by eight sandbox functions; Anthropic rejected it with HTTP 401 authentication_error. This confirms that archived key is unusable, but does not identify why or establish the status of any different key formerly in Secrets Manager. Protected historical backups can still contain credential copies.
...
Changes were initially prepared in an isolated checkout. On follow-up, the user explicitly authorized merging and pushing local work. The original BSKY edits were preserved in commit `43d29cc73` and merged with retirement main; superseded infrastructure edits and the temporary Terraform block were resolved to the retired configuration. The local Terraform state was updated to the verified retired state. PAN’s prior local processing work was already committed and pushed as `c3c8872`. Both local checkouts now include the retirement changes. Old deploy inputs are no longer needed.

## Still unresolved / intentionally deferred

1. **Lambda@Edge cleanup:** AWS propagation cleared and `shadowsky-og-meta-edge` was successfully deleted on the follow-up check. The leftover `ShadowSkyOgMetaEdge` stack and its dedicated IAM role were also deleted and verified absent.
...
4. **Credentials:** AWS credential-store cleanup is complete. The archived sandbox key was tested with explicit user authorization and rejected with HTTP 401 authentication_error; it is not usable. The user reports the keys were already revoked. Any different historical Secrets Manager key was not independently tested. Historical protected backups were preserved.
5. **PAN housekeeping:** the user authorized follow-up removal of the unused Asphodel security group, memory alarm, OOM event rule/target, and that rule’s SNS publishing statement. The group had no interfaces or references. The shared alert topic, subscription, owner permissions, and CloudWatch publishing permission were preserved.
