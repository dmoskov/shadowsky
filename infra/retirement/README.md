...
- `shadowsky/anthropic-api-key` in Secrets Manager and the sandbox `ANTHROPIC_API_KEY` SSM parameter. Credential ownership and shared-value consumers require confirmation before revocation. Removing application/deployment roles did not revoke an external Anthropic key.
...
4. **Credentials:** confirm shared consumers/ownership before revoking the retained Anthropic key/SSM credential. Scoped deployment IAM roles and the Amplify webhook are already removed.
