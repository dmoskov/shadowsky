...
def parse_command(stdin):
    """Parse hook input from stdin and return the command string."""
    hook_data = json.load(stdin)
    return hook_data.get("tool_input", {}).get("command", "").strip()


def check_patterns(command, patterns):
    """Check command against a list of regex patterns. Return first match or None."""
...
def block_command(command, pattern):
    """Print block message and exit with code 2 to prevent execution."""
    print("❌ BLOCKED: This command matches a dangerous pattern.", file=sys.stderr)
...
def warn_command(command):
    """Print warning message for risky but allowed commands."""
    print("⚠️  WARNING: Potentially risky command detected", file=sys.stderr)
    print(f"Command: {command}", file=sys.stderr)
...
def main():
    ...
    critical_match = check_patterns(command, CRITICAL_PATTERNS)
    if critical_match:
        block_command(command, critical_match)

    warning_match = check_patterns(command, WARNING_PATTERNS)
    if warning_match:
