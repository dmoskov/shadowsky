    ...
    if (this.adaptive) {
      if (this.totalRequests > this.slowdownAfter) {
        ...
        const factor = Math.min(
          3,
          1 + (this.totalRequests - this.slowdownAfter) / 100,
        );
...
export async function rateLimitedProfileFetch<T>(
  fn: () => Promise<T>,
): Promise<T> {
...
export async function rateLimitedPostFetch<T>(
  fn: () => Promise<T>,
): Promise<T> {
...
export async function rateLimitedNotificationFetch<T>(
  fn: () => Promise<T>,
): Promise<T> {
