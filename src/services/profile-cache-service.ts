...
export class ProfileCacheService {
   ...
   */
  async getProfilesWithCache(handles: string[]): Promise<Map<string, CachedProfile>> {
   ...
   */
  async getProfilesByDidsWithCache(dids: string[]): Promise<Map<string, CachedProfile>> {
    const db = await getFollowerCacheDB()
    const profileMap = new Map<string, CachedProfile>()
  ...
  async updateInteractionStats(
    notifications: Array<{ author: { did: string, handle: string }, reason: string, indexedAt: string }>
  ): Promise<void> {
    const db = await getFollowerCacheDB()
