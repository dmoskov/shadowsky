...
export class NotificationCacheService {
  ...
  private ensureInitialized(): void {
    if (!this.initialized) {
      throw new Error('NotificationCacheService not initialized. Call init() first.')
    }
  }
  
  // Cache new notifications from a page
  async cacheNotifications(notifications: Notification[], pageNumber: number): Promise<void> {
  ...
  async getCachedNotifications(limit = 100, offset = 0): Promise<NotificationCacheResult> {
    this.ensureInitialized()
  ...
  async getNotificationsByType(reason: string, limit = 100): Promise<Notification[]> {
    this.ensureInitialized()
