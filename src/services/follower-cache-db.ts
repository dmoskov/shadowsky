...
export class FollowerCacheDB {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains("profiles")) {
          ...
          profileStore.createIndex("followersCount", "followersCount", {
            unique: false,
          });
  ...
  async getProfiles(dids: string[]): Promise<Map<string, CachedProfile>> {
    ...
    for (const did of dids) {
      ...
      const profile = await new Promise<CachedProfile | undefined>(
        (resolve, reject) => {
          request.onsuccess = () =>
            resolve(request.result as CachedProfile | undefined);
          request.onerror = () => reject(request.error);
  ...
  async saveMultipleInteractionStats(
    statsList: InteractionStats[],
  ): Promise<void> {
    if (!this.db) {
      debug.error("Database not initialized, cannot save interaction stats");
      return;
  ...
  async getInteractionStats(
    did: string,
  ): Promise<InteractionStats | undefined> {
    if (!this.db) {
      debug.warn("Database not initialized, returning undefined");
      return undefined;
    }
    ...
    const result = await new Promise<InteractionStats | undefined>(
      (resolve, reject) => {
        request.onsuccess = () =>
          resolve(request.result as InteractionStats | undefined);
        request.onerror = () => reject(request.error);
      },
    );
    return result;
  ...
  async getInteractionStatsForMultiple(
    dids: string[],
    ...
    for (const did of dids) {
      const request = store.get(did);
      const stats = await new Promise<InteractionStats | undefined>(
        (resolve, reject) => {
          request.onsuccess = () =>
            resolve(request.result as InteractionStats | undefined);
          request.onerror = () => reject(request.error);
  ...
  async getEnrichedProfiles(
    ...
    const enriched: Array<
      CachedProfile & { interactionStats?: InteractionStats }
    > = [];
  ...
  async getStats(): Promise<{
    ...
    const estimatedSize =
      profileCount * avgProfileSize + interactionCount * avgInteractionSize;
