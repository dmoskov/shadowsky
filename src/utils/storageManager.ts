...
export class StorageManager {
  ...
  static optimizeNotificationStorage(
    ...
    for (const notif of sorted) {
      ...
      if (currentSize + notifSize <= targetSize) {
        ...
        debug.log(
          `📊 Storage optimization: Kept ${compressed.length} of ${notifications.length} notifications`,
        );
        break;
      }
    }
  ...
  static cleanupStorage(keepDays: number = 30): void {
    ...
    if (keysToRemove.length > 0) {
      debug.log(
        `🧹 Cleaned up ${keysToRemove.length} old items, freed ${(removedSize / 1024).toFixed(1)}KB`,
      );
    }
  }
  ...
  static getStorageHealth(): {
    ...
    if (metrics.notificationDataSize > TARGET_STORAGE_SIZE) {
      recommendations.push(
        `Notification data (${(metrics.notificationDataSize / 1024 / 1024).toFixed(2)}MB) exceeds target size`,
      );
    }

    if (metrics.largestItems[0]?.size > 500 * 1024) {
      // Items larger than 500KB
      recommendations.push(
        "Some stored items are very large and could be optimized",
      );
    }
  ...
  static pruneNotifications(
    ...
    // Optimize storage
    const optimized = this.optimizeNotificationStorage(
      allNotifications,
      maxSize,
    );
