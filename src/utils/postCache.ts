...
export class PostCache {
  ...
  private static saveToLocalStorage(posts: Post[]): void {
    try {
      ...
      debug.log(
        `📮 Cached ${posts.length} new posts to localStorage (fallback)`,
      );
    } catch (error) {
  ...
  static getCachedPosts(uris: string[]): { cached: Post[]; missing: string[] } {
    ...
    if (cached.length > 0) {
      debug.log(
        `📮 Found ${cached.length} cached posts out of ${uris.length} requested from localStorage`,
      );
    }
  ...
  private static async checkIndexedDBForMissingPosts(
    uris: string[],
  ): Promise<void> {
    try {
      ...
      if (indexedDBPosts.length > 0) {
        debug.log(
          `📮 Found ${indexedDBPosts.length} posts in IndexedDB that were missing from localStorage`,
        );
   ...
   */
  static async getCachedPostsAsync(
    uris: string[],
    ...
    try {
      ...
      if (indexedDBPosts.length > 0) {
        debug.log(
          `📮 Found ${indexedDBPosts.length} cached posts out of ${uris.length} requested from IndexedDB`,
        );
      }
      ...
      if (localData) {
        ...
        if (additionalPosts.length > 0) {
          debug.log(
            `📮 Found ${additionalPosts.length} additional posts from localStorage`,
          );
        }
