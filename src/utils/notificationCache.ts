...
export class NotificationCache {
  ...
  static save(
    ...
    const notificationCount = pages.reduce(
      (sum, p) => sum + p.notifications.length,
      0,
    );

    debug.log(
      `🟡 [${timestamp}] CACHE SAVE ATTEMPT: ${notificationCount} notifications (priority: ${priority})`,
    );
    ...
    try {
      ...
      const shouldCompress =
        metrics.usagePercentage > 50 || notificationCount > 1000;

      let dataToStore: CachedData;

      if (shouldCompress) {
        debug.log(
          `📦 [${timestamp}] Compressing notifications for efficient storage`,
        );
      ...
      debug.log(
        `📦 [${timestamp}] Data size: ${(dataStr.length / 1024).toFixed(1)}KB, chunks: ${chunks}`,
      );
      ...
      debug.log(
        `✅ [${timestamp}] CACHE SAVED: ${notificationCount} notifications (priority: ${priority})`,
      );
      debug.log(
        `⏰ [${timestamp}] Cache expires: ${expiryDate.toLocaleString()}`,
      );

      // Log storage metrics after save
      const postSaveMetrics = StorageManager.getStorageMetrics();
      debug.log(
        `📊 [${timestamp}] Storage usage: ${postSaveMetrics.usagePercentage.toFixed(1)}% (${StorageManager.formatBytes(postSaveMetrics.totalSize)} / ~5MB)`,
      );
      debug.log(
        `📊 [${timestamp}] Notification data: ${StorageManager.formatBytes(postSaveMetrics.notificationDataSize)}`,
      );
      ...
      if (verification) {
        ...
        debug.log(
          `❌ [${timestamp}] Cache save verification FAILED - no data found in localStorage`,
        );
      }
    } catch (error) {
      ...
      if (
        ...
        try {
          ...
          debug.error(
            `❌ [${timestamp}] Failed to cache after clearing:`,
            retryError,
          );
        }
      }
  ...
  static load(priority?: boolean): CachedData | null {
    ...
    try {
      ...
      if (now > expiryMs) {
        ...
        debug.log(
          `⏰ [${timestamp}] CACHE EXPIRED: expired at ${expiredDate.toLocaleString()} (priority: ${priority})`,
        );
        this.clear(priority);
        return null;
      }
      ...
      debug.log(
        `⏰ [${timestamp}] Cache valid for ${remainingTime} more minutes`,
      );
      ...
      if (chunksCount) {
        debug.log(
          `📦 [${timestamp}] Loading chunked data (${chunksCount} chunks)`,
        );
        // Reassemble chunks
        const chunks: string[] = [];
        ...
        dataStr = chunks.join("");
        debug.log(
          `📦 [${timestamp}] Successfully reassembled ${chunks.length} chunks`,
        );
      } else {
        // Load as single item
      ...
      debug.log(
        `📦 [${timestamp}] Parsing ${(dataStr.length / 1024).toFixed(1)}KB of cached data`,
      );
      ...
      try {
        ...
        debug.error(
          `❌ [${timestamp}] Failed to parse cached data:`,
          parseError,
        );
        debug.log(
          `🔍 [${timestamp}] First 100 chars of invalid data:`,
      ...
      if (cachedData.version !== CACHE_VERSION) {
        debug.log(
          `❌ [${timestamp}] VERSION MISMATCH: expected ${CACHE_VERSION}, got ${cachedData.version}`,
        );
        this.clear(priority);
        return null;
      }

      const notificationCount = cachedData.pages.reduce(
        (sum, p) => sum + p.notifications.length,
        0,
      );
      ...
      debug.log(
        `✅ [${timestamp}] CACHE HIT: ${notificationCount} notifications loaded (priority: ${priority})`,
      );
      debug.log(
        `📊 [${timestamp}] Cache age: ${cacheAge} minutes, ${cachedData.pages.length} pages`,
      );
      ...
      debug.error(
        `❌ [${timestamp}] Failed to load cached notifications:`,
        error,
      );
      this.clear(priority);
      return null;
  ...
  static clear(priority?: boolean): void {
    ...
    try {
      ...
      if (hadData || hadExpiry || chunksCleared > 0) {
        ...
        debug.log(
          `🗑️ [${timestamp}] Cache was already empty (priority: ${priority})`,
        );
      }
    } catch (error) {
  ...
  static getCacheInfo(): {
    ...
    return {
      ...
      hasAllCache: !!allData,
      priorityCacheSize: priorityData
        ? priorityData.pages.reduce((sum, p) => sum + p.notifications.length, 0)
        : 0,
      allCacheSize: allData
        ? allData.pages.reduce((sum, p) => sum + p.notifications.length, 0)
        : 0,
      priorityExpiry: priorityExpiryStr
        ? new Date(parseInt(priorityExpiryStr))
        : null,
