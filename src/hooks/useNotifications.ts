...
export function useNotifications(priority: boolean = false) {
  ...
  if (cachedData) {
    debug.log(
      `🚀 [${timestamp}] React Query: Using cached data as initialData`,
    );
  } else {
    debug.log(
      `🚀 [${timestamp}] React Query: No cache found, will fetch from API`,
    );
  }
  ...
  const query = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }) => {
      ...
      debug.log(
        `✅ [${fetchTimestamp}] React Query: API call completed, got ${result.notifications.length} notifications`,
      );
      return result;
    },
    ...
    getNextPageParam: (lastPage, allPages) => {
      ...
      const totalNotifications = allPages.reduce(
    ...
    initialData: cachedData
      ? {
          ...
          pageParams: [
            undefined,
            ...cachedData.pages.slice(0, -1).map((p) => p.cursor),
          ],
        }
      : undefined,
  ...
  React.useEffect(() => {
    if (query.data?.pages && query.data.pages.length > 0 && !query.isLoading) {
      ...
      const totalNotifications = query.data.pages.reduce(
  ...
  React.useEffect(() => {
    if (query.error) {
      ...
      debug.error(
        `❌ [${errorTimestamp}] React Query error in useNotifications:`,
        query.error,
      );
    }
  }, [query.error]);
