...
export function usePerformanceTracking(componentName: string) {
  ...
  useEffect(() => {
    return () => {
      ...
      analytics.trackTiming('component_lifecycle', 'render_duration', duration, componentName)
    }
...
export function useInteractionTracking() {
  ...
  const trackScroll = useCallback((element: string, scrollPercentage: number) => {
    analytics.trackEvent({
      category: 'ui_interaction',
      action: 'scroll',
      label: element,
...
export function useNotificationTracking() {
  const trackNotificationLoad = useCallback((duration: number, count: number, source: 'cache' | 'api') => {
    analytics.trackNotificationLoad(duration, count, source)
  }, [])

  const trackNotificationInteraction = useCallback((action: string, notificationType: string) => {
...
export function useConversationTracking() {
  const trackConversationView = useCallback((conversationId: string, messageCount: number) => {
