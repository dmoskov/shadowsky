...
export function useReplyNotificationsFromCache() {
  ...
  const cachedReplyNotifications = useQuery({
    ...
    queryFn: () => {
      ...
      debug.log(
        `📨 Found ${replyNotifications.length} reply notifications in extended cache`,
      );
  ...
  const freshReplyNotifications = useReplyNotifications(
    undefined,
    !hasExtendedData,
  );
  ...
  if (hasExtendedData && cachedReplyNotifications.data) {
    return {
      ...
      error: null,
      fetchNextPage: () =>
        Promise.resolve({ data: null, isError: false, isSuccess: true }), // No-op
      hasNextPage: false,
