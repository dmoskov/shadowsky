...
export function useOptimisticPosts() {
  ...
  const updatePostInCaches = (postUri: string, updater: (post: AppBskyFeedDefs.PostView) => AppBskyFeedDefs.PostView) => {
  ...
  const unrepostMutation = useMutation({
    mutationFn: async ({ repostUri }: { repostUri: string; postUri: string }) => {
      if (!agent) throw new Error('Not authenticated')
