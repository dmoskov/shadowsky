...
import { Hash, Loader, RefreshCw, Repeat2 } from "lucide-react";
...
export default function SkyColumnFeed({
  feedUri,
  isFocused = false,
}: SkyColumnFeedProps) {
  const { agent } = useAuth();
  ...
  const renderPost = (feedPost: any, index: number) => {
    ...
    const isFocusedPost =
      hasUserInteracted && focusedPostUriRef.current === post.uri;
      ...
      >
        {isRepost && feedPost.reason?.by && (
          ...
          >
            <Repeat2 size={14} />
            <span>
              {feedPost.reason.by.displayName || feedPost.reason.by.handle}{" "}
              reposted
            </span>
          </div>
        )}
        ...
        <div className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-2 whitespace-pre-wrap break-words text-gray-900 dark:text-white">
                {post.record?.text || ""}
              </div>
  ...
  const PostComponent = React.memo(
    ({ feedPost, index }: { feedPost: any; index: number }) => {
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {feedInfo?.displayName ||
                (feedUri === "following" ? "Following" : "Feed")}
            </h2>
          </div>
          <button
            onClick={() =>
            ...
            className="rounded-lg p-2 transition-all hover:bg-gray-500 hover:bg-opacity-10"
            style={{ color: "var(--bsky-text-secondary)" }}
        ...
        {hasNextPage && (
          <div className="flex justify-center p-4">
            <button
              ...
              className="rounded-lg px-4 py-2 transition-colors disabled:opacity-50"
              style={{
