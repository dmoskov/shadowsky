...
import { StorageOption, StorageOptionSelector } from "./StorageOptionSelector";
...
export const BookmarkStorageSettings: React.FC = () => {
  ...
  useEffect(() => {
    if (appPreferences) {
      setStorageType(
        appPreferences.bookmarkStorageType as "local" | "custom" | "official",
      );
    }
  }, [appPreferences]);
  ...
  const handleStorageChange = async (
    ...
    // No special handling needed for official bookmarks anymore
    if (newType === "official") {
      await performStorageChange(newType);
      return;
    }
  ...
  const getStorageName = (type: "local" | "custom" | "official") => {
    switch (type) {
      case "local":
  ...
  const storageOptions: StorageOption[] = [
    ...
    },
    {
      ...
      icon: Cloud,
      description:
        "Store bookmarks as custom AT Protocol records. WARNING: These are publicly visible to anyone!",
      pros: ["Cross-device sync", "Available now"],
      cons: [
    ...
    {
      type: "official",
      name: "Standard AT Protocol (Private)",
      icon: Cloud,
      description:
        "Private bookmarks using the official Bluesky bookmarks API. Your bookmarks are private and sync across all Bluesky clients.",
      pros: [
        "Private",
        "Cross-device sync",
  ...
  return (
    <div className="space-y-6">
      ...
      <StorageOptionSelector
        ...
        selectedType={storageType as any}
