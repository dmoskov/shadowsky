...
interface CreateCollaborativeListModalProps {
  onClose: () => void;
  onCreate: (request: CreateCollaborativeListRequest) => Promise<void>;
}
...
const ITEM_TYPES: {
...
}[] = [
  {
    ...
    description: "Curate a list of accounts to follow",
  },
  {
    type: "post",
    icon: <Sparkles className="h-5 w-5" />,
    description: "Collect the best posts on a topic",
  },
  {
    type: "topic",
    icon: <Hash className="h-5 w-5" />,
    description: "Organize hashtags and topics",
  },
];

export const CreateCollaborativeListModal: React.FC<
  CreateCollaborativeListModalProps
> = ({ onClose, onCreate }) => {
  const [name, setName] = useState("");
  const [description, setDescription] = useState("");
  const [visibility, setVisibility] = useState<ListVisibility>("public");
  const [itemType, setItemType] = useState<ListItemType>("account");
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="mb-4">
            <label className="mb-2 block text-sm font-medium text-bsky-text-primary">
              What will this list contain?
            </label>
            <div className="grid gap-2">
              {ITEM_TYPES.map(({ type, icon, description: desc }) => (
                <button
                  key={type}
                  type="button"
                  onClick={() => setItemType(type)}
                  className={`flex cursor-pointer items-center gap-3 rounded-lg border p-3 text-left transition-all duration-200 ${
                    itemType === type
                  ...
                  <div
                    className={`rounded-full p-2 ${
                      itemType === type
                        ...
                        : "bg-bsky-bg-tertiary text-bsky-text-secondary"
                    }`}
                  >
                    {icon}
                  </div>
