...
export const DirectMessages: React.FC = () => {
  ...
  useEffect(() => {
    if (conversations === undefined && !loadingConversations) {
      setChatError('Your app password needs chat permissions. Please create a new app password with "Direct Messages" access enabled.')
    }
  }, [conversations, loadingConversations])

  // Fetch messages for selected conversation
  const { data: conversationData, isLoading: loadingMessages, refetch: refetchMessages } = useQuery({
    queryKey: ['dm-conversation', selectedConversation],
    queryFn: () => selectedConversation ? dmService.getConversation(selectedConversation) : null,
  ...
  const getOtherMember = (conversation: DmConversation) => {
    return conversation.members.find(member => member.did !== session?.did) || conversation.members[0]
  }

  return (
    <div className="dm-container h-full">
      {/* Conversations list */}
      <div className={`dm-conversations-list ${selectedConversation ? 'has-selection' : ''}`}>
        <div className="dm-conversations-header">
          <h2 className="text-xl font-semibold" style={{ color: 'var(--bsky-text-primary)' }}>Messages</h2>
        </div>
        ...
        <div className="dm-conversations-scroll">
          {chatError ? (
            ...
            <div className="p-4 text-center" style={{ color: 'var(--bsky-text-secondary)' }}>
                ...
                >
                  <div className="flex items-center">
                    {otherMember.avatar ? (
                      ...
                      <div className="dm-conversation-avatar bg-gray-300 flex items-center justify-center">
                        <span className="text-gray-600 text-lg">
                          {(otherMember.displayName || otherMember.handle || 'U')[0].toUpperCase()}
                        </span>
                      </div>
                    )}
                    <div className="dm-conversation-info">
                      <div className="dm-conversation-name">
                        {otherMember.displayName || otherMember.handle || 'Unknown User'}
                      </div>
                      {otherMember.handle && (
        ...
        {selectedConversation && conversationData ? (
          <>
                ...
                <div className="dm-conversation-avatar bg-gray-300 flex items-center justify-center">
                  <span className="text-gray-600 text-lg">
                    {(getOtherMember(conversationData.conversation).displayName || 
                      getOtherMember(conversationData.conversation).handle || 'U')[0].toUpperCase()}
                  </span>
                </div>
              )}
              <div>
            ...
            <div className="dm-messages-container">
              {loadingMessages ? (
                <div className="text-center" style={{ color: 'var(--bsky-text-secondary)' }}>
                  Loading messages...
                </div>
