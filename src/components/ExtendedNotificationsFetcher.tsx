...
export const ExtendedNotificationsFetcher: React.FC = () => {
  ...
  const cachedStats = React.useMemo(() => {
    ...
    const oldestNotification =
      cachedNotifications[cachedNotifications.length - 1];
    ...
    const loadCachedData = async () => {
      ...
      if (hasCached) {
        ...
        if (cachedResult.notifications.length > 0) {
          ...
          const oldestNotification =
            cachedResult.notifications[cachedResult.notifications.length - 1];
          const oldestDate = new Date(oldestNotification.indexedAt);
          const daysReached = Math.floor(
  ...
  const handleFetch4Weeks = async () => {
    ...
    if (shouldContinue && initialResult.data?.pages) {
      const firstPageNotifications =
        initialResult.data.pages[0]?.notifications || [];
      if (firstPageNotifications.length > 0) {
        const oldestDate = new Date(
          firstPageNotifications[firstPageNotifications.length - 1].indexedAt,
        );
        const daysReached = Math.floor(
          (new Date().getTime() - oldestDate.getTime()) / (1000 * 60 * 60 * 24),
        );
  ...
  const handleFetchMissing = async () => {
    ...
    while (shouldContinue && hasNextPage) {
      ...
      if (latestData?.pages) {
        const allNotifs = latestData.pages.flatMap(
          (page: any) => page.notifications,
        );
      ...
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h2 className="flex items-center gap-2 text-lg font-semibold">
            <Calendar
              className={hasCachedData ? "text-green-500" : "text-blue-500"}
            />
            ...
            >
              <div className="grid grid-cols-2 gap-4 text-sm md:grid-cols-4">
                <div>
                  ...
                  <p className="text-lg font-semibold">
                    {fetchInfo.metadata.totalNotificationsFetched}
                  </p>
                </div>
                <div>
                  <p style={{ color: "var(--bsky-text-secondary)" }}>
                  ...
                  <p className="text-lg font-semibold">
                    {fetchInfo.metadata.daysReached}
                  </p>
                </div>
                <div>
                  <p style={{ color: "var(--bsky-text-secondary)" }}>From</p>
              ...
              >
                ...
                >
                  ...
                  {allNotifications.length > 0
                    ...
                    : 0}{" "}
                  days. The data is now available for all analytics views.
                </p>
              </div>
