...
> **Deployment remains retired.** Amplify, CloudFront hosting, the ECS API service, and the sandbox REST API have been removed. Deployment entrypoints remain disabled. Terraform now manages only retained storage/logs and can plan/apply normally without recreating public infrastructure. PAN feed hosts (`feed.shadowsky.io` and `feed.asphodel.is`), their routing, and processing are preserved. Reopening requires an explicit user decision. See the [proposed retention policy](infra/retirement/RETENTION.md); no data-expiration schedule has been applied.
