 ...
 *
 * Pure logic for grouping notifications (likes, reposts, follows, quotes)
 * into clusters within a time window. Shared by web and mobile so grouping
 * thresholds and reason-mapping never diverge between platforms.
 */
...
export function aggregateNotifications(
  ...
  groups.forEach((groupNotifications, key) => {
    // NOTE: hyphenated reasons ("like-via-repost", "starterpack-joined")
    // truncate to their first segment here. Preserved as-is: filtering still
    // works (the truncated reason is in the same filter bucket) and UIs render
    // the truncated reason with the same copy as the base reason.
