...
export function useUnlikePost() {
  ...
  return useMutation({
    mutationFn: ({likeUri}: {likeUri: string; _postUri: string}) => unlikePost(likeUri),
    ...
    onError: async (_error, {likeUri}, context) => {
      // Rollback optimistic update
      if (context) {
        queryClient.setQueryData(['timeline'], context.previousTimeline);
        context.previousAuthorFeeds.forEach(([key, data]) => {
...
export function useDeleteRepost() {
  ...
  return useMutation({
    mutationFn: ({repostUri}: {repostUri: string; _postUri: string}) =>
    ...
    onError: async (_error, {repostUri}, context) => {
      // Rollback optimistic update
      if (context) {
        queryClient.setQueryData(['timeline'], context.previousTimeline);
        context.previousAuthorFeeds.forEach(([key, data]) => {
