...
describe("ImageCarousel", () => {
  ...
  describe("alt text", () => {
    ...
    it("expands alt text when ALT badge is tapped", () => {
      ...
      const images = [
        {
          thumb: "https://a.jpg",
          fullsize: "https://b.jpg",
          alt: "Unique alt text for overlay",
        },
      ];
  ...
  describe("edge cases", () => {
    it("handles empty images array without crashing", () => {
      expect(() =>
        render(<ImageCarousel {...defaultProps} images={[]} />),
      ).not.toThrow();
    });

    it("handles initialIndex beyond images length", () => {
      expect(() =>
        render(<ImageCarousel {...defaultProps} initialIndex={10} />),
      ).not.toThrow();
    });
  });
});
