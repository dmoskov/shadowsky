...
export default function ProfileRoute() {
  ...
  const handleNavigateToFollowers = (actor: string) => {
    router.push(`/(app)/(tabs)/(home)/followers/${actor}`);
  };

  const handleNavigateToFollowing = (actor: string) => {
    router.push(`/(app)/(tabs)/(home)/following/${actor}`);
  };
  ...
  const handleNavigateToPost = useCallback(
    (uri: string) => {
      ...
      if (postId) {
        router.push(
          `/(app)/(tabs)/(home)/thread/${postId}?handle=${did}&did=${encodeURIComponent(did)}`,
        );
      }
    },
    [router],
  ...
  const handleNavigateToProfile = useCallback(
    (profileHandle: string) => {
      router.push(`/(app)/(tabs)/(home)/profile/${profileHandle}`);
    },
    [router],
  );
