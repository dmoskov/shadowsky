...
async function testSearchTypeaheadFix() {
  ...
  try {
    ...
    await page.goto('http://localhost:5175/search', { waitUntil: 'networkidle0' });
    ...
    if (addUserButton) {
      ...
      if (userInput) {
        ...
        if (dropdown) {
          ...
          const itemCount = await page.$$eval('div.absolute.z-10 button', buttons => buttons.length);
    ...
    if (inputs.length > 0) {
      ...
      console.log(suggestionsExist ? '✓ Suggestions appeared' : '✓ No suggestions (might need auth)');
    }
