...
describe('Search Component Typeahead', () => {
  ...
  test('should not show empty dropdown when pressing arrow down on empty "Add user" input', async () => {
    ...
    await page.goto('http://localhost:5173/search', { waitUntil: 'networkidle0' });
    
    // Wait for search component to load
    await page.waitForSelector('button:has-text("Add user")', { timeout: 5000 });
    
    // Click the "Add user" button
    await page.click('button:has-text("Add user")');
    
    // Wait for the input field to appear
    await page.waitForSelector('input[placeholder*="jay.bsky.team"]', { timeout: 5000 });
    ...
    if (dropdown) {
      ...
      const dropdownContent = await page.$$eval('div[style*="z-index: 10"] button', buttons => buttons.length);
