...
const credentials = getTestCredentials();
  ...
  if (posts.length > 0) {
    ...
    try {
      await page.waitForSelector('.thread-participants-compact', { timeout: 3000 });
      console.log('Thread participants loaded');
    } catch (e) {
      console.log('Thread participants not found - feature may be disabled or no data');
    }
    ...
    if (mapButton) {
      ...
      // Click expand button
      const expandButton = await page.$('.thread-overview-map button[title="Expand map"]');
      if (expandButton) {
