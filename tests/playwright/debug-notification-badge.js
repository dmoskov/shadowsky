...
async function debugNotificationBadge() {
  ...
  try {
    ...
    const badgeInfo = await page.evaluate(() => {
      ...
      if (!badge) {
        return { exists: false, buttonInfo: notificationBtn ? 'Button exists' : 'Button not found' };
      }
      ...
      return {
        ...
        isVisible: badge.offsetParent !== null && 
    ...
    const fullPagePath = path.join(screenshotDir, `notification-debug-full-${timestamp}.png`);
    ...
    if (notificationBtn) {
      const closeupPath = path.join(screenshotDir, `notification-debug-closeup-${timestamp}.png`);
    ...
    if (badgeAfterNav) {
      const afterNavPath = path.join(screenshotDir, `notification-debug-after-nav-${timestamp}.png`);
    ...
    console.log('2. Badge positioned incorrectly: Verify parent has position:relative');
