...
async function interactiveNotificationTest() {
  ...
  try {
    ...
    const needsLogin = await page
      .isVisible('button:has-text("Login")', { timeout: 3000 })
      .catch(() => false);
    ...
    console.log(
      "Button found:",
      analysis.diagnostics.buttonFound ? "✅" : "❌",
    );
    ...
    if (analysis.badge) {
      console.log(
        "Badge visible:",
        analysis.diagnostics.badgeVisible ? "✅" : "❌",
      );
      ...
      console.log("\n📍 Badge Position:");
      console.log(
        `  Absolute: top=${analysis.badge.bounds.top}px, left=${analysis.badge.bounds.left}px`,
      );
      console.log(
        `  Size: ${analysis.badge.bounds.width}x${analysis.badge.bounds.height}px`,
      );
    }
    ...
    if (analysis.button?.position !== "relative") {
      issues.push(
        "Button should have position:relative for proper badge positioning",
      );
    }
