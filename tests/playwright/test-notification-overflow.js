...
async function testNotificationOverflow() {
  ...
  try {
    ...
    const result = await page.evaluate(() => {
      ...
      if (!btn)
        btn = document.querySelector('.nav-icon-button[title="Notifications"]');
      if (!btn) {
        // Find by icon
        ...
        for (const svg of bells) {
          if (
            ...
            svg.parentElement?.title?.includes("Notification")
          ) {
      ...
      return {
        ...
        diagnosis:
          !hiddenVisible && visibleVisible
            ? "overflow:hidden is clipping the badge"
            : "other issue",
      };
    });
    ...
    if (result.error) {
      ...
      console.log(
        "Badge visible with overflow:hidden?",
        result.badgeVisibleWithHidden,
      );
      console.log(
        "Badge visible with overflow:visible?",
        result.badgeVisibleWithVisible,
      );
    ...
    if (!result.error) {
      ...
      await page.screenshot({
        path: "test-screenshots/notification-overflow-hidden.png",
      });
      ...
      await page.screenshot({
        path: "test-screenshots/notification-overflow-visible.png",
      });
