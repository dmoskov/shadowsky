...
async function testNotificationBadge() {
  ...
  try {
    ...
    const loginVisible = await page
      .isVisible('button:has-text("Login")', { timeout: 5000 })
      .catch(() => false);
    ...
    const notificationInfo = await page.evaluate(() => {
      ...
      if (btn) {
        ...
        if (btnStyles.overflow === "hidden") {
          result.issues.push(
            "Button has overflow:hidden which may clip the badge",
          );
        }
        if (
          btnStyles.position !== "relative" &&
          btnStyles.position !== "absolute"
        ) {
          result.issues.push(
            "Button should have position:relative for badge positioning",
          );
        }
      }
    ...
    if (injectedBadgeInfo) {
      ...
      console.log(
        "Is badge within button bounds?",
        injectedBadgeInfo.isWithinButton,
      );
    }
  } catch (error) {
