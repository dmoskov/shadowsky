...
async function testSidebarNavigation() {
  ...
  async function testNavigation(selector, expectedUrl, description) {
    ...
    console.log(
      `  ${success ? "✓" : "✗"} Navigate to ${expectedUrl}: ${success ? "Success" : "Failed"}`,
    );
    return success;
  }
  ...
  try {
    ...
    await capture(
      "sidebar-01-desktop-home",
      "Desktop view with sidebar navigation - Home",
    );
    ...
    await capture(
      "sidebar-03-desktop-notifications",
      "Desktop view - Notifications page",
    );

    // Check for notification badge
    const notificationBadge = await page.$(
      '.nav-item[href="/notifications"] .nav-badge',
    );
    ...
    if (profileLink) {
      ...
      await capture(
        "sidebar-04-desktop-profile",
        "Desktop view - Profile page",
      );
    ...
    if (composeBtn) {
      ...
      await capture(
        "sidebar-06-compose-modal",
        "Compose modal opened from sidebar",
      );
    ...
    await page.waitForTimeout(1000);
    await capture(
      "sidebar-09-mobile-view",
      "Mobile view - Sidebar collapsed/hidden",
    );
    ...
    await page.waitForTimeout(2000);
    await capture(
      "sidebar-15-final-state",
      "Final state - Desktop with sidebar",
    );
...
`;
    ...
    fs.writeFileSync(
      path.join(screenshotDir, "sidebar-test-report.md"),
      report,
    );
