  ...
  test('verify app functionality', async ({ page }) => {
    ...
    if (isLoginPage) {
      ...
      try {
        ...
        console.log('Login form is visible - would need credentials to proceed');
        
        // Check for username/handle input
        const handleInput = await page.locator('input[type="text"], input[placeholder*="handle"], input[placeholder*="username"], input[name*="handle"], input[name*="username"]').first();
      ...
      try {
        ...
        const feedContent = await page.locator('[role="feed"], .feed, main').first();
        ...
        // Check for posts
        const posts = await page.locator('article, [role="article"], .post, .post-card').all();
      ...
      try {
        const composeButton = await page.locator('button:has-text("Compose"), button:has-text("New post"), button:has-text("Post"), button[aria-label*="compose"], button[aria-label*="new"], .compose-button').first();
        if (await composeButton.isVisible()) {
          ...
          const composeArea = await page.locator('textarea, [contenteditable="true"], .compose-area, .compose-modal').first();
          if (await composeArea.isVisible()) {
            ...
            const closeButton = await page.locator('button[aria-label*="close"], button:has-text("Cancel"), .close-button').first();
